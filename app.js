...
(function () {
  ...
  function loadDay(dateISO, skipWeatherAutofill) {
    ...
    // On the very first page load, this is skipped here and run once,
    // deliberately, after the initial cloud sync below has settled — see
    // the comment there for why. On a manual date switch mid-session,
    // there's no competing initial-sync race, so it's safe to run inline.
    if (!skipWeatherAutofill) autoFillWeatherIfBlank();
  }
  ...
  var DAY_MERGE_FIELDS = [
    "wrapUp", "amPriorities", "lockups", "outbounds", "inbounds",
  ...
  function initApp() {
    // Weather auto-fill is deliberately skipped here (see the pullFromCloud
    // callback below for why) and run once, explicitly, after the initial
    // cloud sync has settled instead.
    loadDay(todayISO(), true);
    ...
    // resolves.
    //
    // Weather auto-fill is deliberately deferred until AFTER this resolves,
    // rather than firing immediately in loadDay() on page load. Auto-fill
    // ends in its own save (scheduleSave -> pushToCloud) once the weather
    // API responds — and if that save fired before this initial pull had
    // merged in colleagues' real data, it would overwrite their entire
    // saved sheet with the still-blank local placeholder (plus just the
    // weather). Waiting for this pull to settle first — whether it finds
    // real data, finds nothing, or sync isn't configured at all — means
    // App.daily is guaranteed to reflect the authoritative state before
    // anything is allowed to save over it.
